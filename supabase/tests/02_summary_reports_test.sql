...
begin
  ...
  -- The daily sequence is untouched by any of it.
  select count(*) into n from public.reports where report_number = 1;
  if n < 1 then raise exception 'FAIL numbering: daily report numbering was disturbed'; end if;
end;
$$;
...
begin
  select count(*) into n from public.summary_report_sources
   where report_id is not null and via_summary_report_id is null;
...
begin
  insert into public.summary_report_sources
    (company_id, summary_report_id, report_id, source_summary_report_id)
...
update public.issues
   set status = 'closed', closed_at = now(),
       resolution = 'Materials moved and the run rodded through.'
...
begin
  ...
  select string_agg(coalesce(e.from_status::text, 'none') || '>' || e.to_status::text,
...
select p.company_id, p.id, p.company_id::text || '/' || p.id::text || '/rival.jpg',
       'general', auth.uid()
...
insert into public.issues
  (company_id, project_id, title, status, priority, closed_at, created_at, updated_at)
...
select i.company_id, i.id, null, 'open',
       'Backfilled when issue history was introduced', i.created_at
...
begin
  ...
  select count(*) into n from public.issue_events e
    join public.issues i on i.id = e.issue_id
   where i.title = 'Legacy closed issue'
     and e.to_status = 'closed' and e.from_status = 'open';
